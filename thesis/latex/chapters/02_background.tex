...
The oracle evaluation mode in this thesis treats the attacker's state
(attack type, kill chain stage, intent) as fully observable via the
ground-truth state vector, establishing an upper bound on achievable SEDM
performance. The classifier-driven evaluation mode (Chapter~\ref{ch:results})
replaces the ground-truth attack-type label with the RandomForest
classifier's prediction, introducing exactly this kind of partial
observability; a remaining direction for future work is extending
observability limits to the kill-chain-stage and intent labels as well,
which are still read from ground truth in both evaluation modes.
